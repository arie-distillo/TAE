                ...
                if m and m.definition:
                    ...
                    for _track in _tracks:
                        ...
                        for _det in _track.detections:
                            ...
                            if _u:
                                _img_urls.append(_u)
                                if _crop_path is None:
                                    _ck = _u.split("/tile_img/")[-1] if "/tile_img/" in _u else ""
                                    _crop_path = _tile_img_cache.get(f"path_{_ck}")
...
async def query(message: str):  # noqa — signature only for illustration
    ...
    for track in tracks:
        ...
        for det in track.detections:
            ...
            if url:
                ...
                if crop_path is None:
                    cache_key = url.split("/tile_img/")[-1] if "/tile_img/" in url else ""
                    crop_path = _tile_img_cache.get(f"path_{cache_key}")
        ...
        _state["detections"][det_id] = {
            ...
            "parent_path":     crop_path or best.parent_path,
            "tile_x":          best.tile_x,
            "tile_y":          best.tile_y,
            "tile_w":          best.tile_w,
            "tile_h":          best.tile_h,
            # Task 4: detection provenance scores
...
def _bg_detect_callback(
    ...
    try:
        ...
        for track in all_tracks:
            ...
            _img_urls_bg  = []
            try:
                _u_bg = _annotate_and_save(
                    {"parent_path": best.parent_path,
                     "tile_x": best.tile_x, "tile_y": best.tile_y,
                     "tile_w": best.tile_w, "tile_h": best.tile_h},
                    best.bbox_tile, best.label,
                )
                if _u_bg:
                    _img_urls_bg.append(_u_bg)
                    _ck_bg = _u_bg.split("/tile_img/")[-1] if "/tile_img/" in _u_bg else ""
                    _crop_path_bg = _tile_img_cache.get(f"path_{_ck_bg}")
            except Exception:
                pass
            _state["detections"][det_id] = {
                "lat":             track.lat,    "lon":            track.lon,
                "label":           track.label,  "color":          color,
                "confirmed":       True,         "img_urls":       _img_urls_bg,
                "is_multiangle":   len(track.detections) > 1,
                "source_count":    len(track.detections),
